...
export default class Luna {
  constructor({
    ...
    this.calculateMeanAscendingNode = this.calculateMeanAscendingNode.bind(
      this
    );
    this.calculateMeanDescendingNode = this.calculateMeanDescendingNode.bind(
      this
    );
  }
  ...
  static GetPhaseQuarterString(quarterIndex) {
    switch (quarterIndex) {
      ...
      default:
        throw new Error(
          `Quarter Index: ${quarterIndex} not valid (must be beteen 0 - 3)`
        );
    }
  }

  static GetShapeString({ illuminatedFraction = 0.0 } = {}) {
    if (illuminatedFraction >= 0 && illuminatedFraction < 0.5)
      return "Crescent";
    if (illuminatedFraction >= 0.5 && illuminatedFraction < 1) return "Gibbous";
  }

  static GetShapeDirectionString({ phaseDecimal = 0.0 } = {}) {
  ...
  static GetWithinQuarterApproximation({
    ...
    return (
      (!!phaseDaysBefore && phaseDaysBefore <= halfRangeLength) ||
      (!!phaseDaysPast && phaseDaysPast <= halfRangeLength)
    );
  }
  ...
  static GetQuarterApproximationString({
    ...
    const closestQuarterIndex =
      !!phaseDaysBefore && phaseDaysBefore <= halfRangeLength
        ? util.mod(quarterIndex + 1, 4)
        : util.mod(quarterIndex, 4);
  ...
  static GetQuarterApproximationDirectionString({
    ...
    if (!!phaseDaysBefore && phaseDaysBefore <= halfRangeLength)
      return "Entering";
  ...
  calculateBody(body, earthBody, observer) {
    ...
    console.log("1 ", moonpol);
    ra0 = this.ra;
    dec0 = this.dec;
    lon0 = moonpol[0];
    /* Calculate for present instant.
     */
    body.position.nutation = this.calcll(
      observer.Date.julian,
      moonpp,
      moonpol,
      body,
      earthBody,
    ...
    /* aberration of light. */
    body.position.annualAberration = aberration.calc(
      re,
      earthBody,
      observer,
      body
    ...
    console.log("3 ", moonpol);

    body = util.angles(moonpp, qq, re, body);

    console.log("4 ", moonpol);

    // Jean Meeus Astronomical Algorithims 2015 pg 345
    // illuminatedFraction (k) = (1 + cos(i)) / 2
    // where i = phase Angle of the moon
    ...
    body.position.apparentLongitude =
      body.position.apparentGeocentric.dLongitude;
    ...
    body.position.shapeString = Luna.GetShapeString({
      illuminatedFraction: body.position.illuminatedFraction
    });

    body.position.shapeDirectionString = Luna.GetShapeDirectionString({
      phaseDecimal: body.position.phaseDecimal
    });
  ...
  calculateOrbit(julianDate) {
    ...
    orbit.meanAscendingNode = util.attachApparentLongitudes(
      {},
      getMeanAscendingNode(julianDate)
    );
    orbit.meanDescendingNode = util.attachApparentLongitudes(
      {},
      util.mod(orbit.meanAscendingNode.apparentLongitude - 180, 360)
    );

    orbit.meanPerigee = util.attachApparentLongitudes(
      {},
      getMeanPerigee(julianDate)
    );
    orbit.meanApogee = util.attachApparentLongitudes(
      {},
      util.mod(orbit.meanPerigee.apparentLongitude - 180, 360)
    );
  ...
  calculateMeanAscendingNode(julianDate) {
    ...
    meanAscendingNode.apparentLongitudeString = util.decimalDegreesToDMSString(
      meanAscendingNode.apparentLongitude
    );
    meanAscendingNode.apparentLongitude30String = util.decimalDegreesToDMSString(
      util.mod(meanAscendingNode.apparentLongitude, 30)
    );
  ...
  calculateMeanDescendingNode(ascendingNode) {
    ...
    meanDescendingNode.apparentLongitudeString = util.decimalDegreesToDMSString(
      meanDescendingNode.apparentLongitude
    );
    meanDescendingNode.apparentLongitude30String = util.decimalDegreesToDMSString(
      util.mod(meanDescendingNode.apparentLongitude, 30)
    );
